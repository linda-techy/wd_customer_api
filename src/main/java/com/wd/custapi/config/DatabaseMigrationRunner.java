...
public class DatabaseMigrationRunner implements CommandLineRunner {
    ...
    private void createPasswordResetTokensTable() {
        try {
            ...
            // Repair/align existing schema for secure token storage and stable lookups.
            jdbcTemplate.execute("ALTER TABLE customer_password_reset_tokens ALTER COLUMN reset_code TYPE VARCHAR(128)");
            jdbcTemplate.execute("ALTER TABLE customer_password_reset_tokens ALTER COLUMN used SET DEFAULT FALSE");
            jdbcTemplate.execute("UPDATE customer_password_reset_tokens SET used = FALSE WHERE used IS NULL");
    ...
    private void createIndexIfMissing(String indexName, String tableName, String columns) {
        try {
            String checkSql = "SELECT count(*) FROM pg_indexes WHERE tablename = ? AND indexname = ?";
            ...
            if (count != null && count == 0) {
                ...
                logger.info("Created index '{}' on '{}({})'.", indexName, tableName, columns);
            }
        } catch (Exception e) {
            logger.warn("Failed to ensure index '{}': {}", indexName, e.getMessage());
        }
    }
}
